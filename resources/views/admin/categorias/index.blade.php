        ...
        <div class="row">
            ...
            <div class="col-8 col-md-3 col-lg-4">
                <form action="{{ route('admin.categorias.index') }}">
                    ...
                    @if(request('s'))
                        <a href="{{ route('admin.categorias.index') }}" class="text-danger">Limpar busca <i class="fas fa-times"></i></a>
                    @endif
                </form>
            </div>
                        ...
                        <table class="table table-hover shadow-sm">
                            ...
                            @foreach($listagem as $row)
                                <tr {{ $row->ativo ? '' : 'class=table-danger' }}>
                                    <td><a href="{{ route('admin.categorias.show', ['id' => $row->id]) }}">{{ $row->categoria }}</a></td>
                                    <td>
